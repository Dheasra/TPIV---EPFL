...
for N in range(2, Nmax+1):
    ...
    tar_rslt.append(an_val[Target]/shots)
    err_tar.append(an_val[Target]/shots) #Values 1.15e-2 and 2.31e-2 are respectively the average CNOT and average readout errors
    nse_rslt.append(max(an_val_alt)/shots)
    err_nse.append((1.15e-2 + 2.31e-2)*max(an_val_alt)/shots) #Values 1.15e-2 and 2.31e-2 are respectively the average CNOT and average readout errors


N_list = range(2,Nmax+1)

plt.errorbar(N_list, tar_rslt, err_tar, None,'b+')
plt.errorbar(N_list, nse_rslt, err_nse, None,'r+')
plt.grid(True)
